...
INSERT OVERWRITE TABLE covid_ds_partitioned PARTITION(COUNTRY_NAME)
 SELECT Country,Total_Cases,New_Cases,CASE WHEN TRIM(Total_Deaths)="" THEN "0" ELSE Total_Deaths END,
 New_Deaths,Total_Recovered,Active_Cases,Serious,Tot_Cases,Deaths,CASE WHEN TRIM(Total_Tests)="" THEN "0" ELSE Total_Tests END ,
...
INSERT OVERWRITE TABLE covid_final_output PARTITION(COUNTRY_NAME)
 ...
 DENSE_RANK() OVER (ORDER BY CAST(regexp_replace(regexp_replace(TRIM(total_deaths),'\\.00',''),',','') as int) DESC ),
 DENSE_RANK() OVER (ORDER BY CAST(regexp_replace(regexp_replace(TRIM(total_tests),'\\.00',''),',','') as int) DESC ),
